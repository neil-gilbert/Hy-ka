    ...
    def generate(self, task_input: str, model_config: dict) -> ProviderResult:
        ...
        if not self._api_key or not self._endpoint:
            return ProviderResult(
                ...
                cost_usd=0,
                raw_response={},
                error="AZURE_OPENAI_API_KEY or AZURE_OPENAI_ENDPOINT is not configured",
            )

        try:
            client = AzureOpenAI(
                api_key=self._api_key,
                api_version=model_config.get("api_version", self._api_version),
                azure_endpoint=model_config.get("endpoint") or self._endpoint,
            )
            ...
            completion = client.chat.completions.create(
                model=model_config.get("model_name_override") or model_config.get("model_name"),
                messages=messages,
            ...
            return ProviderResult(
                ...
                cost_usd=0,
                raw_response={},
                error=f"azure_openai_error: {exc}",
            )
